...
docker-build: ## Build a container image and add the version and latest tag
	@if command -v docker > /dev/null; then \
		echo "Building..."; \
		docker buildx build -t jsfelipearaujo/ms-order-management:latest -t jsfelipearaujo/ms-order-management:$$(git rev-parse --short HEAD) .; \
	else \
...
docker-push: ## Push the container image to the registry
	echo "Pushing..."; \
	docker push jsfelipearaujo/ms-order-management:latest && \
	docker push jsfelipearaujo/ms-order-management:$$(git rev-parse --short HEAD)
